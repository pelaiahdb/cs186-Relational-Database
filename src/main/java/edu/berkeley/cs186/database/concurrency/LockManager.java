...
public class LockManager {
  ...
  private void handlePotentialDeadlock(Lock lock, long transNum, LockType lockType) {
    ...
    for (Long owner : owners) {
      if (!waitsForGraph.edgeExists(transNum, owner)) {
        if (lock.getType().equals(LockType.EXCLUSIVE)) {
          if (!owners.contains(transNum) && transNum != owner) {
            waitsForGraph.addNode(transNum);
            waitsForGraph.addEdge(transNum, owner);
            if (waitsForGraph.edgeCausesCycle(transNum, owner)) {
              throw new DeadlockException("Deadlock Exception caught");
            }
          }
        }
        if (lockType.equals(LockType.EXCLUSIVE) && transNum != owner) {
          waitsForGraph.addNode(transNum);
          waitsForGraph.addEdge(transNum, owner);
          if (waitsForGraph.edgeCausesCycle(transNum, owner)) {
            throw new DeadlockException("Deadlock Exception caught");
